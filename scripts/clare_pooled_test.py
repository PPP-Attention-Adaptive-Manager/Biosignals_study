"""
clare_pooled_test.py
======================
Closes the three gaps identified in the previous CLARE analysis:

  1. LEVEL-AGNOSTIC POOLED TEST replaces the level0-vs-level3 contrast
     as the primary result. Label-ordering check (clare_label_check_v2.py)
     found only 10/19 participants (53%) show monotonically increasing
     self-reported load across level_0->level_3 -- essentially a coin
     flip. Treating "level" as a clean difficulty ladder and pooling
     across all participants was not supported by the data. This test
     pools ALL experiment-phase windows regardless of level number,
     asking the more fundamental question: does instantaneous HR/RMSSD
     predict instantaneous frontal EEG at all, without depending on the
     complexity manipulation having landed cleanly for everyone.

  2. MODEL COMPARISON: CCA, RF, XGBoost, CatBoost side by side. The
     previous CCA-only result (frontal CV r=0.267 +/- 0.485) had SD
     larger than the mean -- comparing multiple model families is how
     every other part of this project (SENSE-42, HEP) distinguished a
     genuine effect from small-sample CCA instability.

  3. PERMUTATION CONTROL on every model. Settles directly whether
     SD > mean reflects real per-participant heterogeneity in coupling
     strength (plausible, and would be a genuine finding) or is
     ordinary noise around zero (the null).

STILL RUNS THE TEMPORAL-SITE CONTROL
----------------------------------------
TP9/TP10 alongside AF7/AF8, same logic as before: if temporal matches
or beats frontal, that is an artifact warning independent of CFA (CLARE
uses separately-recorded hardware, so this would indicate something
else -- generic arousal-linked broadband EEG change, not cardiac field
artifact specifically).

WINDOWING FOR THE POOLED TEST
----------------------------------
Sliding 30s windows (50% overlap) within each experiment-phase
recording, pooled across all 4 levels per participant -- more data
points per participant than the previous one-row-per-phase-level
design, needed for LOSO folds to be stable enough for the permutation
test to mean anything.

Run from: ~/biosignals_data/
Output:   outputs/clare_pooled_results.json
"""
...
OUT_JSON = os.path.join(BASE, "outputs", "clare_pooled_results.json")
